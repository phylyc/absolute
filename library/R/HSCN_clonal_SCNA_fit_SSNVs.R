GetHscnSomaticMutComb <- function(alpha, q1, q2, nc)
{
  q.a <- c(1:q2)
  q <- q1 + q2
  f.reads <- alpha * q.a / (nc * (1 - alpha) + alpha * q)
  vals <- q.a
...
# eqn. 11
GetHscnGermlineMutComb <- function(alpha, q1, q2, nc) 
{
  eps <- 0.001
  
  q.vals <- c(q1, q2, q1 + q2)
  Q <- q1 + q2
  f.reads <- ((1 - alpha) + alpha * q.vals) / (nc * (1 - alpha) + alpha * Q)
  f.reads[3] <- 1 - eps
  vals <- q.vals
  res <- cbind(vals, f.reads)
...
{
  hap.q.keys <- paste(mut.cn.dat[, "HS_q_hat_1"], mut.cn.dat[, "HS_q_hat_2"], mut.cn.dat[,"normal_allele_count"],
                      sep="__" )
  ...
  for(i in seq_len(n.keys))
  {
    ...
    q2 <- as.integer(res[2])
    nc = as.integer(res[3])
    mut.ix <- (mut.cn.dat[, "HS_q_hat_1"] == q1) & (mut.cn.dat[, "HS_q_hat_2"] == q2) & (mut.cn.dat[,"normal_allele_count"] == nc)
...
#    m.ix = mut.cn.dat[mut.ix, "male_X"]
    ...
    som.comb <- GetHscnSomaticMutComb(alpha, q1, q2, nc)
