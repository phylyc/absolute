...
##
  ...
  SSNV_model[["epsilon"]] = 1e-3  ## seq error-rate
...
log_density_alt_cond_coverage = function( alt, cov, f, SSNV_model)
{
   ...
   return(l_dens)
#   d_real = d_beta_binom( c(0:cov), A, B, cov, log=FALSE )
#   d_err = dbinom( c(0:3), cov, eps/3, log=FALSE )
#   d_conv = convolve( d_real, rev(d_err), type="open")
}
...
#  o.log.pr <- matrix(NA, nrow = length(alt), ncol = length(f.comb))
...
#    o.log.pr[,i] = d_beta_binom( alt, A, B, cov, log=TRUE )

    log.pr[,i] = log_density_alt_cond_coverage( alt, cov, f.comb[i], SSNV_model )
  }
...
#  B[B < 0] = 0.0    ## fix rounding errors leading to small negative numbers
  ...
  {
#    oldbb = d_beta_binom( alt[j], A, B, cov[j], log=TRUE )
#    oldbb[ outer_prod == 0 ] = ifelse( alt[j]==0, log(1), log(0) )
#    oldbb[ outer_prod == 1 ] = ifelse( ref[j]==0, log(1), log(0) )

    ldbb = matrix( log_density_alt_cond_coverage( alt[j], cov[j], outer_prod, SSNV_model ), nrow=nrow(outer_prod))
...
#  B[B < 0] = 0.0    ## fix rounding errors leading to small negative numbers
  ...
  {
#    oldbb = d_beta_binom( alt[j], A, B, cov[j], log=TRUE )
#    oldbb[ AF_mat == 0 ] = ifelse( alt[j]==0, log(1), log(0) )
#    oldbb[ AF_mat == 1 ] = ifelse( ref[j]==0, log(1), log(0) )

    ldbb = matrix( log_density_alt_cond_coverage( alt[j], cov[j], AF_mat, SSNV_model ), nrow=nrow(AF_mat))
...
calc_ccf_posterior_LL_grid = function(alt, ref, SSNV_model, alpha, q, normal_allele_count ) 
{
#  f_skew =  SSNV_model[["SSNV_skew"]]
#  rho =  SSNV_model[["rho"]]
  grid = SSNV_model[["ccf_grid"]]

  f = (alpha * grid) / (normal_allele_count * (1 - alpha) + alpha * q)
  cov = alt + ref
...
#  A = use_f * rho
#  B = (1-use_f) * rho
#  oll_grid <- d_beta_binom( alt, A, B, cov, log=TRUE )


  ll_grid = log_density_alt_cond_coverage( alt, cov, f, SSNV_model )
