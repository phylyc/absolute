...
{
   get_cluster_log_Z = function( c_ix, log_t_data, log_prior  )
   {
      log_f_LL = rowSums( log_t_data[,c_ix,drop=FALSE]) + log_prior
      return(LogAdd(log_f_LL))
   }
...
#      pred[i-N_burn+1,] = colSums( mat ) / (sum(DP_res$c_N[[i]]) + sum(fixed_scales) )
   ...
   cluster_log_Z = rep(NA, length(CL))
   for( i in 1:length(CL))
   {
      c_ix = assign==CL[i]
      cluster_dens[i,] = get_cluster_stats( c_ix, log(t(data)), log(clust_CCF_prior) )
      cluster_log_Z[i] = get_cluster_log_Z( c_ix, log(t(data)), log(clust_CCF_prior) )
   }

   assign = assign[1:N_mut]
## TODO: sort clusters by CCF mode

   return( list("assign"=assign, "CCF_dens"=cluster_dens, "cluster_log_Z"=cluster_log_Z))
}
