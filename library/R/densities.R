...
## version in Base R used until  06.18.2015
original_LogAdd <- function(X) {
...
# Version that uses C implementation in package "matrixStats" for %20 speedup with NaN / NA support
LogAdd = function(X) 
{
   if( is.matrix(X) ) {
      val = rowLogSumExps(X)
...
{
    stats::dt((x - mu)/sigma, nu, log=log )
}

p_scaled_t = function( x, mu, sigma, nu, log=FALSE )
{
#    stats::pt((x - mean)/sigma, nu, ncp = ncp, log.p = FALSE)
    stats::pt((x - mu)/sigma, nu, log.p=log )
}

# not used anymore, equiv to above
SLC_d_scaled_t =  function( x, mu, sigma, nu, log=FALSE )
{
