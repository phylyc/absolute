...
#      lines(grid[i, ], mut_pr[i, ], col=mut_colors[i])
  ...
  if (draw_total)
  {
#    lines(mult_grid, colSums(clonal_dens*pr_clonal), col=cols[2], lty=4)
#    lines(mult_grid, colSums(sc_dens*pr_subclonal), col=cols[1], lty=4)
...
SSNV_CCF_plot = function( mut.dat, SSNV_ccf_dens, mode.ix, SSNV_cols, max_SSNVs_plot )
{
### downsample SSNVs to plot:
  if( nrow(mut.dat) > max_SSNVs_plot ) 
  {   
     ix = sample( c(1:nrow(mut.dat)), max_SSNVs_plot )
  }
  else{ ix = c(1:nrow(mut.dat)) }
...
SSNV_on_subclonal_SCNA_CCF_plot = function( mut.dat, SSNV_ccf_dens, mode.ix, max_SSNVs_plot )
{
### downsample SSNVs to plot:
  if( nrow(mut.dat) > max_SSNVs_plot ) 
  {   
...
plot_SSNVs_on_genome = function( SSNV_model, SSNV_colors, mut.dat, seg.dat, i, mode.color, max_SSNVs_plot = 500, verbose=FALSE )
{
   ...
   plot_SSNV_genome_vs_CI95 = function( mut.dat, colors, CI95, max_N )
   {
      if(nrow(mut.dat) > max_N)
      {
         ix = sample( c(1:nrow(mut.dat)), max_N )
      }
      else{ ix = c(1:nrow(mut.dat))}
...
## plot up to this number of each mutation type on genome
   max_N = ceiling(max_SSNVs_plot / 2)
   ...
   if( any(!SC_CN.ix)) 
   {
      ...
      plot_SSNV_genome_vs_CI95( mut.dat[!SC_CN.ix,,drop=FALSE], colors, C_mult_ci95, max_N )
   }
   ...
   if( any( SC_CN.ix ))
   {
      ...
      for( i in 1:(dim(H.123.ssnv.ccf.dens)[1]) )
      {
         ...
         TRANS = 200 * mut.H.Prs[,i]
         colors = rgb( r, g, b, TRANS, maxColorValue=255 )

         plot_SSNV_genome_vs_CI95( mut.dat[SC_CN.ix,,drop=FALSE], colors, SC_mult_ci95, max_N )
...
plot_SSNV_on_subclonal_SCNA_CCF_summaries = function(mut.dat, seg.dat, mode.ix, min.cov=1, max_SSNVs_plot=500, verbose=FALSE)
{
  cov <- mut.dat[, "alt"] + mut.dat[, "ref"]
  ix <- cov > min.cov & mut.dat[,"alt"] > 0
  mut.dat <- mut.dat[ix, , drop=FALSE]
  ...
  if (length(names(SSNV_ccf_dens_temp))!=0){
 	 SSNV_ccf_dens=as.data.frame(matrix(SSNV_ccf_dens_temp,ncol=length(names(SSNV_ccf_dens_temp))))
  	 colnames(SSNV_ccf_dens) = names(SSNV_ccf_dens_temp)
  } else {
	  SSNV_ccf_dens=as.data.frame(matrix(SSNV_ccf_dens_temp,ncol=ncol(SSNV_ccf_dens_temp)))
	  colnames(SSNV_ccf_dens) = colnames(SSNV_ccf_dens_temp)
  }
  SSNV_ccf_dens = SSNV_ccf_dens[ix,,drop=FALSE]
...
## 4th plot: CCF of muts on subclonal SCNAs by H* class
  SSNV_on_subclonal_SCNA_CCF_plot( mut.dat, SSNV_ccf_dens, mode.ix, max_SSNVs_plot )
}
