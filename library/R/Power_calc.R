...
get_min_CCF_for_pow <- function(required.power, coverage, eps, fdr, delta, SSNV_model, min_CCF=0.01, dCCF=0.001, single_read=FALSE) 
{
  if (delta <= 0 | delta > 1) {
    return(NA)
  }

  if(min_CCF <= 0) { stop("min_CCF must be > 0") }

  CCF_GRID = seq( min_CCF, 1.0, by=dCCF )

  if( single_read ) 
  {
     pow_grid = PowerCalc_for_single_read( coverage, CCF_GRID * delta, SSNV_model)
  }
  else
  {
     pow_grid = PowerCalc( coverage, eps, fdr, CCF_GRID * delta, SSNV_model)
  }

  ix = which(pow_grid >= required.power)
  if( length(ix) > 0 )
  { 
     min_CCF = CCF_GRID[ min(ix) ]
  } 
  else{ min_CCF = NA }  ## no CCF values reach the required power at the given coverage
