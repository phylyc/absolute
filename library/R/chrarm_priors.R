...
GetChrArmMat <- function(mode.res, mode, max.q ) 
{
  chr.arm.tab = mode.res[["mode_SCNA_models"]][[mode]][["chr.arm.tab"]]
  chrarm.mat = get_modal_chrarm_CN( chr.arm.tab, max.q )
...
get_modal_chrarm_CN = function( chr.arm.tab, max.q, n.arm=39 )
{
  if (dim(chr.arm.tab)[1] == 2) {
