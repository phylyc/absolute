...
# ev params
#   SCNA_model[["chrarm_1CN_penalty"]] = 1
#   SCNA_model[["chrarm_WGD_penalty"]] = c(0,5,20)   ## penalties for 0, 1, and >1 WGD event
#   SCNA_model[["subclone_penalty"]] = -2
   SCNA_model[["NA_chrarm_penalty"]] = -10
...
#  SCNA_model[["clonal_CCF_bins"]] = c(1,NGRID)
  SCNA_model[["kQ"]] = 8 
...
comb_dens = function( d, mu, seg_sigma, log, ... )
{
## rescale seg-level sd to maintain variance under t-distr
   nu = 10 
   seg_sigma = sqrt( seg_sigma^2 * (nu-2)/nu )
   result = d_scaled_t( d, mu, seg_sigma, nu, log=log )

#   result = dnorm( d, mu, seg_sigma, log=log )

   return(result)
}

comb_CDF = function( mu, d, seg_sigma, ... )
{
   nu = 10
   seg_sigma = sqrt( seg_sigma^2 * (nu-2)/nu )
   result = p_scaled_t( mu, d, seg_sigma, nu )

#   result = pnorm( mu, d, seg_sigma )
...
#print(msg1)
  ...
  msg2 <- substitute(paste( hat(theta[Z]) == x3, ", ", 
			    hat(theta[Q]) == x4, ", ",
                            "WGD=", x5, sep=""),
                     list( x3 = round(mode.info["theta.z.hat"], 2),
                           x4 = round(mode.info["theta.Q.hat"], 3),
                           x5 = mode.info["WGD"] ))
  
#print(msg2)

  msg3 <- substitute(paste("SCNA LL" == x6, ", ", "SSNV LL" == x7, ", ", "SCNA Ev" == x8, 
                           sep = ""),
                     list(x6 = round(mode.info["SCNA_LL"], 2),
                          x7 = round(mode.info["SSNV_LL"], 2), 
                          x8 = mode.info["SCNA_min_chrarm_events"] )) 
