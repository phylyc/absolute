...
#  segobj.list = agg_res[["segobj.list"]]
  ...
  if (colnames(dat)[3] == "sample") {
     ...
     segobj.list = segobj.list[found]
     match.dat = MatchPpModes(pp.calls, segobj.list, pp.calls_ploidy_colname, ploidy_colname)
...
MatchPpModes <- function(pp.calls, segobj_list, pp.calls_ploidy_colname, ploidy_colname)
{
   ...
   for (i in seq_len(N)) 
   {
      ...
      if ((any(is.na(pp.calls[sid, c("purity", pp.calls_ploidy_colname)]))) || (!is.na(seg.dat[["mode.res"]][["mode.flag"]]))) { next }

      called_purity = pp.calls[sid, "purity"] 
      called_ploidy = pp.calls[sid, pp.calls_ploidy_colname] 

      match.dat[i,] <- MatchPpMode(mode.tab, called_purity, called_ploidy, ploidy_colname)
    }
    return(match.dat)
}


MatchPpMode <- function(mode.tab, called_purity, called_ploidy, ploidy_colname, ploidy_tol=0.25, purity_tol=0.05)
{
  vals <- mode.tab[, c("alpha", ploidy_colname), drop=FALSE]   
  
  eps.N.ix = (abs(called_ploidy - vals[,ploidy_colname]) <= ploidy_tol) &
             (abs(called_purity - vals[,"alpha"]) <= purity_tol)

  if( sum( eps.N.ix) > 0 )
  {
    mode.ix = which(eps.N.ix)[ which.max( mode.tab[eps.N.ix, "combined_LL"] ) ]
    best_ploidy = mode.tab[mode.ix,ploidy_colname]
  ...
  else
  {
    ...
    call.vals <- c(called_purity, called_ploidy)
    ...
    best_ploidy = vals[min.mode, ploidy_colname]
    best_purity = vals[min.mode, "alpha"]
  }

  return( c(mode.ix, best_ploidy, best_purity, called_ploidy, called_purity) )
}
