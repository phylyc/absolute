...
DP_mut_posterior_plots = function( alt_tab, loc_res, GRID, CCF_dens, SID, assign=NULL, remove_unknown=FALSE )
{   
   if( remove_unknown )
   {
...
## sort mutations by 1st order of posterior CCF dist
   ...
   if( !is.null(assign)) { assign = assign[order(E_loc)] }
   ...
   {
      if( !is.null(assign) ) { mut_col = assign[i]+1 }
      else{ mut_col = ifelse( alt_tab[i,"type"] == "SCNA", "cyan4", "maroon" ) }
...
DP_post_K_plot= function( DP_res, N_burn )
{
## Domain of apprx. to k
   GRID = DP_res[["DP_prior"]][["k_0_map"]][,"k_0"]
...
#   N_iter = length(DP_post$c_fpost)
   N_iter = length(DP_res[["DP_post"]][["K"]]) - 1
