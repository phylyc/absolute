...
#   multi_VCF[,,"ssnv.biallelic"] = get_mut_count_matrix( MAF, gene_list, ordered_sample_names )
   ...
   if( "PhyloP46_cons" %in% colnames(MAF) )
   {
      nix = is.na(MAF[,"PhyloP46_cons"])
      MAF[nix,"PhyloP46_cons"] = -Inf 
      multi_VCF[,,"PhyloP46_cons"] = aggregate_numeric_MAF_column( MAF, gene_list, ordered_sample_names, "PhyloP46_cons", max)
   }

   if( "trinuc_context_num" %in% colnames(MAF) )
   {
      nix = is.na(MAF[,"trinuc_context_num"])
      MAF[nix,"trinuc_context_num"] = 0  
      multi_VCF[,,"trinuc_context_num"] = aggregate_numeric_MAF_column( MAF, gene_list, ordered_sample_names, "trinuc_context_num", max)
   }
...
{
   required_cols = c("pair_id", "Hugo_Symbol", "Number of times codon change is in COSMIC"  )
   if( !all( required_cols %in% colnames(agg_MAF) ) ) 
   {
      print("Missing required agg_MAF cols!") 
      print( setdiff( required_cols, colnames(agg_MAF) ) )
      stop()
   }
...
#   annot_names = c("var_class", "COSMIC_site_count", "ssnv.biallelic", "clinical actionability priority", TF_annot )
   ...
   if( "trinuc_context_num" %in% colnames(agg_MAF) )
   {
      annot_names = c(annot_names, "trinuc_context_num" ) 
   }

   if( "PhyloP46_cons" %in% colnames(agg_MAF) )
   {
      annot_names = c(annot_names, "PhyloP46_cons" ) 
   }
...
#   filtered_MAF[,"Hugo_Symbol"] = as.character(filtered_MAF[,"Hugo_Symbol"])
#   o.ix = order( filtered_MAF[, c("pair_id", "Hugo_Symbol", "Variant_Classification_Num")], decreasing=TRUE )
   o.ix = base:::order( filtered_MAF[, "pair_id"], filtered_MAF[, "Hugo_Symbol"], filtered_MAF[,"Variant_Classification_Num"], decreasing=TRUE )
