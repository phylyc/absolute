...
#just use model num of clusters
   use_k = rr[,2][which.max(rr[,1])]
...
## For segs excluded due to high sem, fill in a regularized CCF density estimate
  ...
  ## Don't save large field with CCF dens = 'c_fpost'
   trunc = DP_res[["DP_post"]][c("K", "eta", "gamma", "assign", "c_fhat", "c_W", "c_N")] 
   DP_res[["DP_post"]] = trunc
   SCNA_model[["DP_res"]] = DP_res
...
## get a table with distr over components for all segs included
   ...
   if(K==1) { seg_clust_tab[,1] = 1; return(seg_clust_tab) }
...
#   male_X = obs[["male_X"]]
   ...
   amp_ix[!male_X] = obs[["d.tx"]][!male_X] > max(comb_A)
   neg_ix[!male_X] = obs[["d.tx"]][!male_X] < min(comb_A)

   amp_ix[male_X] = obs[["d.tx"]][male_X] > max(comb_X)
   neg_ix[male_X] = obs[["d.tx"]][male_X] < min(comb_X)
...
# regularize total CN CCF estimates using predictive DP density learned from allelic data
   ...
   SCNA_model[["tot.CN_states"]] = tot.CN_states
...
#      tot.obs = extract_total_copy_ratios_from_allelic_CAPSEG( seg.obj )  
      SCNA_model = apply_allelic_model_to_total_copy_ratios( seg.obj[["obs.total.scna"]], b, delta, SCNA_model )
   }
