...
#  male_X = obs[["male_X"]]
  ...
  if( any(male_X))
  {
    ...
    clonal_seg_LL_X = sapply(comb_X, comb_dens, d[male_X], use_sigma[male_X], log=TRUE)
...
get_comb_interval_gaussian_CDF_seg_mat = function( obs, b, delta, SCNA_model )
{
  get_int_mat = function( d, comb, sigma )
  {
     ...
     for( i in 1:length(d))
     {
        mid = diff( comb_CDF(comb, d[i], sigma[i] ) )
#        mid = diff( pnorm(comb, d[i], sigma[i] ) )

        neg = comb_CDF( comb[1], d[i], sigma[i] )
        amp = 1 - comb_CDF( comb[Q], d[i], sigma[i] )
