...
#  if(modal_cn==0) { modal_cn = 1 } ## don't model state of 0, screws up lots of calculations
  ...
  obs[["male_X"]] = obs[["normal_allele_count"]] == 1     
  is.X = obs[["male_X"]] 

## Autosomes
  cr_set = obs$d.tx[!obs[["male_X"]] ] 
  res = calc_firstorder_subclonal_states(cr_set, modal_cn, comb_A, neg.ix[!is.X] )
  qc_A = res$qc
  qs_A = res$qs
...
## X-chr male?
  if( any(obs[["male_X"]]))
  {
    cr_set = obs$d.tx[is.X]  
    res = calc_firstorder_subclonal_states(cr_set, modal_cn, comb_X, neg.ix[is.X] )
    qc_X = res$qc
    qs_X = res$qs
...
{
  if( !(length(cr_set)==length(neg.ix)) ) { stop("wtf?") }
...
#    male_X = obs[["male_X"]]
   ...
   for( i in 1:N)
   {
      ...
      pow[i,] = 1 - comb_CDF( y_05, cr_grid[i,], seg_sigma[i] )
   } 
...
GetScnaStderrGridDensity <- function(obs, grid, SCNA_model, i=NA) {
   ...
   seg_sigma = get_seg_sigma( SCNA_model, obs )
#   grid.dens[i, ] = comb_dens( grid, obs[["d.tx"]][i], seg_sigma[i], log=TRUE)
   ...
   if( is.na(i))
   {
      ...
      grid.dens = comb_dens( grid, obs$d.tx[i], seg_sigma[i], log=TRUE)
   }
...
## don't allow clonal homozygous deletions over regions with SSNVs having > 0 alt reads.
      ...
      if( any(homdel.ix) )
      {
         mut_seg_ix = mut.cn.dat[homdel.ix,"T.seg.ix"]
